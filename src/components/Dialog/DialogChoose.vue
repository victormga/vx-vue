<template>
	<div class="vx-dialog-item vx-elevated">
		...
		<div class="vx-dialog-options">
			<vx-row>
				<vx-column v-for="opt in (options.options ?? [])" :key="opt.value">
					<vx-button class="vx-dialog-option" :class="opt.btnClass ?? []" @click="choose(opt)">
						<vx-icon v-if="opt.icon" :icon="opt.icon" />
						{{ opt.label }}
					</vx-button>
				</vx-column>
			</vx-row>
...
export default class DialogChoose extends VxVue {
	declare options: DialogOptions;

	choose(opt: Record<string, unknown>): void {
		this.$emit("done", opt.value);
	}
}
</script>
